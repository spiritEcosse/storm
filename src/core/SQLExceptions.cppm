...
public:
  ConnectionNotOpenException()
      : ConnectionException("Database connection is not open") {}
};
...
public:
  TransactionAlreadyActiveException()
      : TransactionException("Transaction already in progress") {}
};

class TransactionNotActiveException : public TransactionException {
public:
  TransactionNotActiveException()
      : TransactionException("No active transaction") {}
};
...
public:
  explicit SQLQueryException(const std::string &message,
...
public:
  explicit InvalidColumnException(const std::string &columnName,
                                  const std::string &query = "")
      : SQLQueryException("No such column: " + columnName, query),
...
public:
  explicit NullStatementException(const std::string &query = "")
      : SQLQueryException(
            "Cannot execute a null statement. Preparation likely failed.",
            query) {}
};

class StatementPreparationException : public SQLQueryException {
public:
  explicit StatementPreparationException(const std::string &error,
                                         const std::string &query = "")
...
public:
  explicit StatementExecutionException(const std::string &error,
                                       const std::string &query = "")
