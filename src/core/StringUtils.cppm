...
export namespace storm::utils {
    ...
    template <std::size_t N> struct fixed_string {
        ...
        template <typename F> consteval auto transform(F&& func) const {
        ...
        template <typename Pred> consteval auto filter(Pred&& pred) const {
            fixed_string<N> result{};
        ...
        template <typename F> consteval auto and_then(F&& func) const {
            return func(*this);
        }
    };
    ...
    template <std::size_t I, typename... Args> consteval auto get_nth_arg(Args&&... args) {
        return std::get<I>(std::forward_as_tuple(args...));
    }

    // Simple concat function with fixed maximum size
    template <typename... Args> consteval auto concat(Args&&... args) {
        ...
        auto append = [&](const auto& arg) {
            auto sv = [&] {
                if constexpr (requires { arg.view(); })
                    return arg.view();
                else
    ...
    template <std::size_t MaxSize = 1024> struct string_builder {
        fixed_string<MaxSize> buffer{};
        std::size_t           current_pos = 0;

        consteval string_builder() = default;

        template <typename T> consteval auto append(T&& value) const {
    ...
    template <std::size_t Size = 1024> consteval auto make_string_builder() {
        return string_builder<Size>{};
    }

    // Enhanced compile-time string transformations
    namespace transforms {
        constexpr auto to_lower = [](char c) consteval { return (c >= 'A' && c <= 'Z') ? c + 32 : c; };

        constexpr auto to_upper = [](char c) consteval { return (c >= 'a' && c <= 'z') ? c - 32 : c; };

        constexpr auto is_alpha = [](char c) consteval { return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'); };

        constexpr auto is_digit = [](char c) consteval { return c >= '0' && c <= '9'; };
    } // namespace transforms

    // Variadic template with parameter pack indexing (C++26)
    template <typename... Strings> consteval auto join_with(std::string_view separator, Strings&&... strings) {
    ...
    consteval auto formatFieldName(const TableName& table, const FieldName& field) {
        return make_string_builder<256>().append("\"").append(table).append("\".\"").append(field).append("\"").build();
    }

    // Compile-time regex-like pattern matching (simple version)
    template <fixed_string Pattern> struct ct_pattern {
        static constexpr auto pattern = Pattern;

        consteval bool matches(std::string_view input) const {
            // Simple wildcard matching (* and ?)
            return matches_impl(input.begin(), input.end(), pattern.view().begin(), pattern.view().end());
        }

      private:
        consteval bool matches_impl(auto input_it, auto input_end, auto pattern_it, auto pattern_end) const {
    ...
    template <fixed_string Pattern> consteval auto make_pattern() {
