...
private:
   ...
   */
  [[nodiscard]] std::expected<void, std::string>
  bind_primary_keys(std::span<const T> objects) noexcept {
  ...
  build_sql(size_t count) const noexcept {
    return std::format(
        "{} WHERE {} IN ({}){}", get_base_delete_sql(),
   ...
   */
  [[nodiscard]] std::expected<std::string, std::string>
  build_sql() const noexcept {
    return std::format(
        "{}{}{}", get_base_delete_sql(),
        this->_where_clause
            ? std::format(" WHERE {}", this->_where_clause->to_query().sql)
            : "",
        get_returning_count_clause());
  }
};
