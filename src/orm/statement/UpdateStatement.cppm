...
public:
    explicit UpdateStatement(std::shared_ptr<Connection> conn) 
        : Base(conn) {}
...
private:
        ...
        for (const auto& obj : objects) {
            ...
            result = this->bind_primary_key_field(obj, param_index)
                .and_then([this, &obj, &param_index]() -> std::expected<void, std::string> {
                    // Then bind all field values for the THEN (?, ?) part using compile-time filtering
                    return this->bind_fields_with_descriptors(obj, param_index, Base::field_is_not_primary_key_predicate);
                });
    ...
    std::string build_case_clauses(size_t num_objects) const {
        ...
        for (size_t i : std::views::iota(0u, num_objects)) {
            std::format_to(std::back_inserter(clauses), 
                "WHEN {} = ? THEN ? ", this->get_primary_key_field());
        }
        return clauses;
    }
    ...
    constexpr std::string build_placeholders(size_t count) const {
        ...
        // Manual implementation since std::views::join_with is not available in libc++
        std::string result;
