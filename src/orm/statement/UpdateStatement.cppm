 ...
 */
template <typename T>
class UpdateStatement : public UnifiedStatementBase<UpdateStatement<T>, T> {
private:
...
public:
  ...
  execute(std::span<const T> objects) noexcept {
    if (objects.empty()) {
      return 0;
    }
    ...
    return build_sql(objects.size())
        .and_then(
            [this](const std::string &sql) -> std::expected<void, std::string> {
...
private:
  ...
  bind_where_parameters() noexcept {
   ...
   */
  [[nodiscard]] std::expected<void, std::string>
  bind_parameters(std::span<const T> objects) noexcept {
    int param_index = 1;
    ...
    for (const auto &obj : objects) {
      ...
      result =
          this->bind_primary_key_field(obj, param_index)
              .and_then([this, &obj,
                         &param_index]() -> std::expected<void, std::string> {
  ...
  get_returning_count_clause() const noexcept {
    return "";
  }
  ...
  get_base_update_sql() noexcept {
    ...
    constexpr std::size_t total_size =
        prefix.size() + table_name.size() + suffix.size() + 1;
  ...
  build_sql(size_t num_objects) const noexcept {
    ...
    for (const auto &field_name : field_names) {
      ...
      std::format_to(std::back_inserter(sql), "{} = CASE {} ELSE {} END",
                     field_name, build_case_clauses(num_objects), field_name);
    }

    // Add WHERE and RETURNING clauses
    std::format_to(std::back_inserter(sql), " WHERE {} IN ({}){}",
  ...
  std::string build_case_clauses(size_t num_objects) const {
    ...
    for (size_t i : std::views::iota(0u, num_objects)) {
      std::format_to(std::back_inserter(clauses), "WHEN {} = ? THEN ? ",
                     this->get_primary_key_field());
    }
    return clauses;
  }
