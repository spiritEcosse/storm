...
export namespace storm {
    ...
    class Row {
      ...
      private:
        ...
        std::vector<int>         column_types; // Stores SQLite column types (SQLITE_INTEGER,
                                               // SQLITE_FLOAT, etc.)
    };
    ...
    constexpr bool is_bindable_type_v =
            std::is_arithmetic_v<T> || std::is_same_v<T, std::string> || std::is_same_v<T, std::string_view> ||
     ...
     */
    template <typename Derived, typename T> class UnifiedStatementBase {
      public:
      ...
      public:
        ...
        template <typename Member> using MemberDescriptorPredicate = bool (*)(Member);

        // Helper to get field name from member descriptor
        template <typename MemberDesc> static consteval std::string_view get_field_name(const MemberDesc& member) {
        ...
        // Bind fields using member descriptors directly - enables true compile-time
        // filtering
        ...
        // Execute a query and return a single row (for SELECT statements expecting
        // one result)
         ...
         */
        template <typename Value> storm::SqlValue to_sql_value(Value&& value) {
            ...
            // Handle string types
            else if constexpr (std::is_same_v<CleanType, std::string> || std::is_same_v<CleanType, std::string_view> ||
      ...
      protected:
        ...
        std::expected<void, std::string>
        bind_value(int& param_index, const ValueType& value, std::string_view member_name = "") {
    ...
    template <typename Derived, typename T> UnifiedStatementBase<Derived, T>::~UnifiedStatementBase() {
        if (stmt) {
    ...
    template <typename Derived, typename T>
    UnifiedStatementBase<Derived, T>&
    UnifiedStatementBase<Derived, T>::operator=(UnifiedStatementBase&& other) noexcept {
        if (this != &other) {
            if (stmt)
                sqlite3_finalize(stmt);
    ...
    template <typename Derived, typename T> void UnifiedStatementBase<Derived, T>::bind(int idx, const char* value) {
        sqlite3_bind_text(stmt, idx, value, -1, SQLITE_TRANSIENT);
    }

    template <typename Derived, typename T> void UnifiedStatementBase<Derived, T>::bind_null(int idx) {
    ...
    template <typename Derived, typename T> void UnifiedStatementBase<Derived, T>::reset() {
        sqlite3_reset(stmt);
        sqlite3_clear_bindings(stmt);
    }

    template <typename Derived, typename T> int UnifiedStatementBase<Derived, T>::column_int(int idx) const {
        return sqlite3_column_int(stmt, idx);
    }

    template <typename Derived, typename T> double UnifiedStatementBase<Derived, T>::column_double(int idx) const {
        return sqlite3_column_double(stmt, idx);
    }

    template <typename Derived, typename T> std::string UnifiedStatementBase<Derived, T>::column_text(int idx) const {
    ...
    template <typename Derived, typename T>
    std::expected<void, std::string>
    UnifiedStatementBase<Derived, T>::bind_primary_key_field(const T& obj, int& param_index) {
        constexpr auto pk_member = refl::reflect<T>::get_primary_key_member();
    ...
    template <typename Derived, typename T>
    std::expected<SqlValue, std::string>
    UnifiedStatementBase<Derived, T>::get_bindable_value(const T& obj, std::string_view field_name) {
        std::expected<SqlValue, std::string> result = std::unexpected{std::format("Field '{}' not found", field_name)};
        ...
        refl::reflect<T>::visit_members(obj, [&](std::string_view member_name, const auto& value) {
            if (member_name == field_name) {
                ...
                if constexpr (is_bindable_type_v<ValueType>) {
                    ...
                    result = std::unexpected{std::format("Field '{}' has unsupported type for binding", field_name)};
                }
            }
        });
    ...
    template <typename Derived, typename T>
    std::expected<bool, std::string> UnifiedStatementBase<Derived, T>::bind_parameters(const ParameterBinder& binder) {
