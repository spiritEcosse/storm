...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const UnifiedStatementBase &statement) {
    os << statement.sql();
    return os;
  }
...
public:
  ...
  static constexpr auto field_is_not_primary_key_predicate =
      [](auto member) consteval {
  ...
  [[nodiscard]] static consteval auto get_filtered_fields() {
    return refl::reflect<T>::get_member_names_if(
        field_is_not_primary_key_predicate);
  }
  ...
  std::expected<void, std::string> bind_primary_key_field(const T &obj,
                                                          int &param_index);
  ...
  bind_fields_with_descriptors(const T &obj, int &param_index,
                               FieldFilter filter);

  // Get field value as a variant for dynamic binding
  std::expected<SqlValue, std::string>
  get_bindable_value(const T &obj, std::string_view field_name);

  // Bind parameters from expressions
  std::expected<bool, std::string>
  bind_parameters(const ParameterBinder &binder);
  ...
  [[nodiscard]] int
  get_parameter_index(const std::string_view param_name) const {
    return sqlite3_bind_parameter_index(
        stmt, (":" + std::string(param_name)).c_str());
  }
  ...
  template <typename Value> storm::SqlValue to_sql_value(Value &&value) {
    ...
    // Handle enums
      ...
      return static_cast<int>(
          static_cast<std::underlying_type_t<CleanType>>(value));
    }
    // Unsupported type
    else {
...
protected:
  ...
  std::expected<void, std::string>
  bind_value(int &param_index, const ValueType &value,
...
template <typename Derived, typename T>
UnifiedStatementBase<Derived, T>::UnifiedStatementBase(
    UnifiedStatementBase &&other) noexcept
...
template <typename Derived, typename T>
UnifiedStatementBase<Derived, T> &UnifiedStatementBase<Derived, T>::operator=(
    UnifiedStatementBase &&other) noexcept {
  if (this != &other) {
    if (stmt)
      sqlite3_finalize(stmt);
...
template <typename Derived, typename T>
std::expected<void, std::string>
UnifiedStatementBase<Derived, T>::set_sql(const std::string &sql) {
  ...
  int rc =
      sqlite3_prepare_v2(this->conn->get(), sql_.c_str(), -1, &stmt, nullptr);
  if (rc != SQLITE_OK) {
    std::string errMsg = sqlite3_errmsg(this->conn->get());
    return std::unexpected(
        std::format("Failed to prepare statement: {}", errMsg));
  }
...
template <typename Derived, typename T>
std::expected<Row, std::string>
UnifiedStatementBase<Derived, T>::execute_query() {
  int rc = sqlite3_step(stmt);
  if (rc == SQLITE_ROW) {
...
template <typename Derived, typename T>
std::expected<std::vector<Row>, std::string>
UnifiedStatementBase<Derived, T>::execute_all() {
  std::vector<Row> rows;
...
template <typename Derived, typename T>
std::expected<void, std::string>
UnifiedStatementBase<Derived, T>::bind_primary_key_field(const T &obj,
                                                         int &param_index) {
...
template <typename FieldFilter>
std::expected<void, std::string>
UnifiedStatementBase<Derived, T>::bind_fields_with_descriptors(
...
template <typename Derived, typename T>
std::expected<SqlValue, std::string>
UnifiedStatementBase<Derived, T>::get_bindable_value(
  ...
  refl::reflect<T>::visit_members(
      obj, [&](std::string_view member_name, const auto &value) {
        if (member_name == field_name) {
          ...
          if constexpr (is_bindable_type_v<ValueType>) {
            ...
            result =
                SqlValue{static_cast<std::underlying_type_t<ValueType>>(value)};
          } else if constexpr (std::is_convertible_v<ValueType, std::string>) {
...
template <typename Derived, typename T>
std::expected<bool, std::string>
UnifiedStatementBase<Derived, T>::bind_parameters(
  ...
  for (const auto &[param_name, param_value] : parameters) {
    ...
    auto bind_result =
        bind_variant(*static_cast<Derived *>(this), param_index, param_value);
    if (!bind_result) {
      return std::unexpected("Failed to bind parameter '" + param_name +
                             "': " + bind_result.error());
    }
  }
