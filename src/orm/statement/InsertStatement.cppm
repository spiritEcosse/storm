 ...
 */
template <typename T>
class InsertStatement : public UnifiedStatementBase<InsertStatement<T>, T> {
private:
...
public:
  ...
  execute(std::span<const T> objects) noexcept {
    ...
    return build_sql(objects.size())
        ...
        .and_then([](const auto &rows)
                      -> std::expected<std::vector<int>, std::string> {
          return rows | std::views::transform([](const auto &row) {
...
private:
  ...
  build_sql(size_t num_objects) const noexcept {
    ...
    return std::expected<std::string, std::string>{[&]() -> std::string {
      ...
      std::string placeholders_template =
          create_placeholders(field_names.size());
