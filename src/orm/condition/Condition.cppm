...
// ParameterBinder is imported via storm.expression which imports
// storm.parameter_binder SqlValue, Op, and Collation are now imported from
// storm.core_types
...
public:
  ...
  Condition(std::string field_name, Op op, T &&value,
      ...
      : field_name_(std::move(field_name)), operator_(op),
        value_(std::forward<T>(value)), collation_(collation) {}

  template <typename T1, typename T2>
  Condition(std::string field_name, T1 &&value1, T2 &&value2,
            Collation collation = Collation::NONE) // BETWEEN
...
private:
  ...
  std::string value_to_sql_safe(const SqlValue &val,
