...
std::string_view collation_to_sql(Collation collation) {
    ...
    switch (collation) {
        ...
        case NONE: return "";
        default: throw UnknownCollationException();
    }
}
...
std::string_view op_to_sql(Op op) {
    ...
    switch (op) {
        ...
        default: throw UnknownOperatorException();
    }
}
...
std::string Condition::value_to_sql_safe(const SqlValue& val, ParameterBinder& binder) const {
    return std::visit([&binder]<typename T>(const T& v) -> std::string {
