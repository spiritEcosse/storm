...
public:
  // Constructors
  QuerySet() = default;
  explicit QuerySet(std::shared_ptr<Connection> connection)
  ...
  QuerySet(const QuerySet &other)
      : conn(other.conn), _whereExpression(other._whereExpression),
        join_clauses(other.join_clauses), orderTerms(other.orderTerms),
        _jsonFields(other._jsonFields), functionsSet(other.functionsSet),
        _limit(other._limit), _offset(other._offset), _one(other._one),
        _doAndCheck(other._doAndCheck), _returnInMain(other._returnInMain) {
  ...
  QuerySet<T> &
  group_concat(std::string_view alias = "", std::string_view separator = ",",
               std::string_view fieldSeparator = " ", bool distinct = false);
  ...
  template <auto Field>
  std::expected<typename member_pointer_traits<decltype(Field)>::type,
                std::string>
  max_value();
  ...
  insert(const std::vector<T> &objs);
  std::expected<std::vector<int>, std::string>
  insert(std::span<const T> objects);
...
private:
  ...
  execute_update(std::span<const T> objects) const noexcept {
    if (objects.empty())
      return false;
  ...
  [[nodiscard]] std::expected<bool, std::string>
  execute_delete(std::span<const T> objects) const noexcept {
    if (objects.empty())
      return false;
  ...
  [[nodiscard]] std::string buildFieldsClause() const {
    ...
    if (!distinctFields.empty() && onlyFields.empty()) {
      ...
      auto field_strings =
          onlyFields | std::views::transform([](const auto &desc) {
      ...
      fields_clause =
          std::format("{}", storm::utils::join(function_clauses, ", "));
  ...
  build_combined_clause(const std::vector<std::string> &field_strings,
  ...
  [[nodiscard]] storm::Where
  create_in_condition(const FieldType &field_obj,
                      const Container &values) const {
    if (values.empty()) {
      // Return a condition that's always false for empty IN clause
      return storm::Where(
          std::make_unique<storm::Condition>("1", storm::Op::EQ, 0));
    }
  ...
  QuerySet<T> &add_order_field(bool ascending, Collation collation) {
    ...
    this->orderTerms.push_back(
        OrderTerm{d.table, d.field, ascending, collation});
  ...
  template <typename ReturnType, typename SetupFunction,
            typename ValueExtractor = std::nullptr_t>
  std::expected<ReturnType, std::string>
  execute_aggregate_query(SetupFunction setup_function,
...
QuerySet<T> &QuerySet<T>::where(const storm::Where &where_clause) {
  if (this->_whereExpression) {
    ...
    this->_whereExpression =
        std::make_shared<storm::Where>(*this->_whereExpression && where_clause);
  } else {
...
QuerySet<T> &QuerySet<T>::where(storm::Where &&where_clause) {
  if (this->_whereExpression) {
    ...
    this->_whereExpression =
        std::make_shared<storm::Where>(std::move(where_clause));
  }
  return *this;
}
...
QuerySet<T> &QuerySet<T>::where(Value &&value, storm::Op op) {
  ...
  storm::Where condition =
      create_condition(field_obj, std::forward<Value>(value), op);
...
QuerySet<T> &QuerySet<T>::where_between(T1 &&value1, T2 &&value2) {
  ...
  storm::Where condition =
      field_obj.between(std::forward<T1>(value1), std::forward<T2>(value2));
  return where(std::move(condition));
}
...
QuerySet<T>::update(const std::vector<T> &objs) {
  return execute_update(std::span<const T>{objs});
}
...
template <typename T>
std::expected<bool, std::string>
QuerySet<T>::update(std::span<const T> objects) {
...
template <typename T>
std::expected<std::vector<int>, std::string>
QuerySet<T>::insert(std::vector<T> objs) {
...
template <typename T>
std::expected<std::vector<int>, std::string>
QuerySet<T>::insert(const std::vector<T> &objs) {
  return execute_insert(std::span<const T>{objs});
}
...
template <typename T>
std::expected<std::vector<int>, std::string>
QuerySet<T>::insert(std::span<const T> objects) {
...
template <typename T>
std::expected<bool, std::string>
QuerySet<T>::remove(const std::vector<T> &objs) {
...
QuerySet<T> &QuerySet<T>::order_by() {
  ...
  // Add the first field-direction pair
  {
    auto d = make_field_desc<Field>();
    this->orderTerms.push_back(
        OrderTerm{d.table, d.field, Direction, Collation::NONE});
  }
...
template <auto FirstField, auto... RestFields>
std::pair<std::string, std::string>
QuerySet<T>::prepare_group_concat(std::string_view alias,
                                  std::string_view fieldSeparator) {
  // Validate all member pointers
  static_assert(std::is_member_pointer_v<decltype(FirstField)>,
                "FirstField must be a member pointer");
  ...
  std::string actual_alias =
      alias.empty() ? std::format("group_concat_{}", firstDesc.field)
                    : std::string(alias);

  // Build field expression
  std::string field_expr =
      build_field_expression<FirstField, RestFields...>(fieldSeparator);
...
QuerySet<T> &QuerySet<T>::group_concat_with_order_impl(
    std::string_view alias, std::string_view separator,
    std::string_view fieldSeparator, bool distinct) {
  static_assert(std::is_member_pointer_v<decltype(OrderField)>,
                "OrderField must be a member pointer");

  auto [actual_alias, field_expr] =
      prepare_group_concat<FirstField, RestFields...>(alias, fieldSeparator);
...
template <auto FirstField, auto... RestFields>
std::string
QuerySet<T>::build_field_expression(std::string_view fieldSeparator) {
  auto firstDesc = make_field_desc<FirstField>();
  ...
  (
      [&field_expr, &fieldSeparator]<auto Field>() {
        auto d = make_field_desc<Field>();
        field_expr = std::format("{}||'{}'||{}", field_expr, fieldSeparator,
                                 d.full_name());
...
QuerySet<T> &QuerySet<T>::max(std::string_view alias) {
  ...
  // aggregate functions
  functionsSet.emplace_back(
      Function(std::format("MAX({}) AS {}", desc.full_name(), actual_alias)));
  return *this;
}
...
QuerySet<T> &QuerySet<T>::min(std::string_view alias) {
  ...
  // aggregate functions
  functionsSet.emplace_back(
      Function(std::format("MIN({}) AS {}", desc.full_name(), actual_alias)));
  return *this;
}
...
QuerySet<T> &QuerySet<T>::avg(std::string_view alias) {
  ...
  static_assert(std::is_arithmetic_v<FieldType> &&
  ...
  // aggregate functions
  functionsSet.emplace_back(
      Function(std::format("AVG({}) AS {}", desc.full_name(), actual_alias)));
  return *this;
}
...
QuerySet<T> &QuerySet<T>::count(std::string_view alias) {
  ...
  // aggregate functions
  functionsSet.emplace_back(
      Function(std::format("COUNT({}) AS {}", desc.full_name(), actual_alias)));
  return *this;
}
...
QuerySet<T> &QuerySet<T>::sum(std::string_view alias) {
  ...
  functionsSet.emplace_back(
      Function(std::format("SUM({}) AS {}", desc.full_name(), actual_alias)));
  return *this;
}
...
template <auto FirstField, auto... RestFields>
QuerySet<T> &
QuerySet<T>::group_concat(std::string_view alias, std::string_view separator,
                          std::string_view fieldSeparator, bool distinct) {
...
QuerySet<T> &QuerySet<T>::join() {
  ...
  const std::string lhs_field = std::format(
      "{}_{}", storm::utils::to_lower(std::string{rhs_table}), rhs_field);

  const std::string lhs_full =
      storm::utils::formatFieldName(lhs_table, lhs_field);
  const std::string rhs_full =
      storm::utils::formatFieldName(rhs_table, rhs_field);

  join_clauses.emplace_back(
      std::format("INNER JOIN {} ON {} = {}", rhs_table, lhs_full, rhs_full));
  return *this;
}
...
QuerySet<T> &QuerySet<T>::left_join() {
  ...
  join_clauses.emplace_back(
      std::format("LEFT JOIN {} ON {} = {}", rhs_table, lhs_full, rhs_full));
  return *this;
}
...
template <typename T> std::expected<T, std::string> QuerySet<T>::select_one() {
  try {
    ...
    return std::unexpected("Exception in select_one(): " +
                           std::string(e.what()));
  }
}
...
template <typename T> ExpectedValueVectorMap QuerySet<T>::select_values() {
  try {
    ...
    if (!typed_result) {
      return std::unexpected("Failed to execute select_values query: " +
                             typed_result.error());
    }
    ...
    return std::unexpected("Exception in select_values(): " +
                           std::string(e.what()));
  }
}
...
std::expected<ReturnType, std::string>
QuerySet<T>::execute_aggregate_query(SetupFunction setup_function,
                                     std::string_view error_prefix,
...
std::expected<typename member_pointer_traits<decltype(Field)>::type,
              std::string>
QuerySet<T>::max_value() {
  using FieldType = typename member_pointer_traits<decltype(Field)>::type;

  return execute_aggregate_query<FieldType>(
      [](auto &qs) { qs.template max<Field>(); }, "find maximum value");
}
...
std::expected<typename member_pointer_traits<decltype(Field)>::type,
              std::string>
QuerySet<T>::min_value() {
  using FieldType = typename member_pointer_traits<decltype(Field)>::type;

  return execute_aggregate_query<FieldType>(
      [](auto &qs) { qs.template min<Field>(); }, "find minimum value");
}
...
std::expected<double, std::string> QuerySet<T>::avg_value() {
  return execute_aggregate_query<double>(
      [](auto &qs) { qs.template avg<Field>(); }, "average");
}
...
std::expected<int, std::string> QuerySet<T>::count_value() {
  return execute_aggregate_query<int>(
      [](auto &qs) { qs.template count<Field>(); }, "count");
}
...
std::expected<double, std::string> QuerySet<T>::sum_value() {
  return execute_aggregate_query<double>(
      [](auto &qs) { qs.template sum<Field>(); }, "sum");
}
