...
export import storm_orm_ref_action; // RefAction, ref_action_sql (#562) — re-exported so
                                    // storm::meta::RefAction / storm::RefAction are unchanged
...
export namespace storm::meta {
    ...
    // Caller-assigned integer primary key (#586). Declares that the caller supplies
    // the key value on INSERT — the database does not auto-generate it. Both
    // GENERATED BY DEFAULT AS IDENTITY (PG) and plain INTEGER PRIMARY KEY (SQLite)
    // accept an explicit value, so the DDL is identical to plain Primary; the
    // difference is on the DML side: the PK column IS emitted in INSERT and there
    // is no RETURNING clause.
    struct PrimaryAssigned {};
    ...
    inline constexpr PrimaryAssigned      primary_assigned{};
    ...
    // RefAction / ref_action_sql live in the storm_orm_ref_action leaf module (#562),
    // re-exported above — a consumer that only needs the FK/junction ON DELETE policy
    // (e.g. storm_orm_relation_meta) does not need to pull in the rest of this file.
    ...
    // opt-in, #379), `primary_assigned` (caller-assigned integer PK, #586), or
    // `primary_part` (one column of a composite key, #500). Every PK-detection site
    // routes through here so the variants can never drift apart.
    consteval auto is_primary_member(std::meta::info member) -> bool {
        return has_annotation_type<Primary>(member) || has_annotation_type<PrimaryAutoincrement>(member) ||
               has_annotation_type<PrimaryAssigned>(member) || is_primary_part_member(member);
    }
