...
consteval FieldDescView make_field_desc_ct() {
  ...
  return {refl::reflect<ClassType>::get_struct_name(), field,
          std::string_view{Alias}};
}
...
[[nodiscard]] inline FieldDesc make_field_desc() {
  ...
  return FieldDesc{std::string(v.table), std::string(v.field),
                   std::string(v.alias)};
}

// Owning wrapper with runtime alias (table/field names at CT, alias at RT)
template <auto MemberPtr>
[[nodiscard]] inline FieldDesc make_field_desc(std::string alias) {
