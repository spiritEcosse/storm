...
private:
    ...
    // Simplified member name lookup - use a placeholder for now to fix compilation
...
public:
    ...
    template<auto MP> requires std::is_same_v<decltype(MP), decltype(MemberPtr)>
    ...
    template<typename T>
    constexpr Where operator==(T&& value) const {
    ...
    // Special operators
    template<typename T>
    constexpr Where like(T&& pattern) const {
        return Where(std::make_unique<Condition>(
            std::string{field_name}, 
            Op::LIKE, 
    ...
    template<typename T>
    constexpr Where is(T&& value) const {
        return Where(std::make_unique<Condition>(
            std::string{field_name}, 
            Op::IS, 
            std::forward<T>(value), 
    ...
    constexpr auto with_collation(this Self&& self, Collation collation) noexcept {
