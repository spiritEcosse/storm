...
concept StringViewConvertible =
    requires { std::string_view{std::declval<T>()}; };
...
private:
  ...
  // Simplified member name lookup - use a placeholder for now to fix
  // compilation
  static constexpr std::string_view member_name = "field_name";

  // Pre-computed field name at compile time - simplified for now
  static constexpr std::string_view field_name = member_name;

  Collation collation_ = Collation::NONE;

public:
  // C++23: Default constructor
  constexpr explicit Field(Collation collation = Collation::NONE) noexcept
      : collation_(collation) {}

  // Constructor that takes a member pointer for deduction
  template <auto MP>
    requires std::is_same_v<decltype(MP), decltype(MemberPtr)>
  constexpr explicit Field(decltype(MP),
                           Collation collation = Collation::NONE) noexcept
      : collation_(collation) {}
  ...
  template <typename T> constexpr Where operator==(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::EQ, std::forward<T>(value), collation_));
  }

  template <typename T> constexpr Where operator!=(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::NE, std::forward<T>(value), collation_));
  }

  template <typename T> constexpr Where operator>(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::GT, std::forward<T>(value), collation_));
  }

  template <typename T> constexpr Where operator<(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::LT, std::forward<T>(value), collation_));
  }

  template <typename T> constexpr Where operator>=(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::GE, std::forward<T>(value), collation_));
  }

  template <typename T> constexpr Where operator<=(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::LE, std::forward<T>(value), collation_));
  }

  // Special operators
  template <typename T> constexpr Where like(T &&pattern) const {
    return Where(std::make_unique<Condition>(std::string{field_name}, Op::LIKE,
                                             std::forward<T>(pattern),
                                             collation_));
  }

  constexpr Where is_null() const {
    return Where(std::make_unique<Condition>(std::string{field_name}, Op::IS,
                                             std::nullopt, collation_));
  }

  constexpr Where is_not_null() const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::IS_NOT, std::nullopt, collation_));
  }

  template <typename T> constexpr Where is(T &&value) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, Op::IS, std::forward<T>(value), collation_));
  }

  template <typename T1, typename T2>
  constexpr Where between(T1 &&value1, T2 &&value2) const {
    return Where(std::make_unique<Condition>(
        std::string{field_name}, std::forward<T1>(value1),
        std::forward<T2>(value2), collation_));
  }

  // IN operator for multiple values - C++23 enhanced
  template <typename Container>
  constexpr Where in(const Container &values) const {
    if consteval {
      // C++23: Compile-time branch for empty containers
      if (std::ranges::empty(values)) {
        return Where(std::make_unique<Condition>(std::string{field_name},
                                                 Op::EQ, "__IMPOSSIBLE_VALUE__",
                                                 collation_));
      }
    }
  ...
  template <typename T>
  constexpr Where in(std::initializer_list<T> values) const {
    return in(std::span<const T>(values));
  }

  // StringViewConvertible concept now defined at namespace scope
    ...
    requires StringViewConvertible<T>
  {
    ...
    auto pattern =
        std::format("{}%", std::string_view{std::forward<T>(prefix)});

    return Where(std::make_unique<Condition>(std::string{field_name}, Op::LIKE,
                                             std::move(pattern), collation_));
  }
    ...
    requires StringViewConvertible<T>
  {
    ...
    // and fall back to runtime evaluation when necessary
    auto pattern =
        std::format("%{}", std::string_view{std::forward<T>(suffix)});
  ...
  constexpr auto with_collation(this Self &&self,
                                Collation collation) noexcept {
