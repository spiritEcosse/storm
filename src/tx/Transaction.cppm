...
public:
  explicit Transaction(std::shared_ptr<Connection> conn,
                       Connection::TransactionLevel level =
                           Connection::TransactionLevel::DEFERRED)
  ...
  ~Transaction() {
    if (active_) {
      try {
        ...
        std::cerr << "Error during automatic rollback in destructor: "
                  << e.what() << std::endl;
      }
    }
  }
    ...
    if (this != &other) {
      if (active_) {
        try {
          ...
          std::cerr << "Error rolling back transaction during move: "
                    << e.what() << std::endl;
        }
      }
...
decltype(auto) with_transaction(std::shared_ptr<Connection> conn, Func &&func,
                                Connection::TransactionLevel level =
                                    Connection::TransactionLevel::DEFERRED) {
