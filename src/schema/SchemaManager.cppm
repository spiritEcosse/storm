...
private:
  ...
  std::vector<std::string>
      create_table_sqls; // Store SQL strings instead of functions
  std::unordered_set<std::string> created_tables;

  template <typename T> static std::string get_sql_type() {
  ...
  template <typename T> std::string generate_create_table_sql() {
    ...
    refl::reflect<T>::for_each_member([&]<size_t I>(auto member) {
      ...
      std::string field_type_sql =
          get_sql_type<typename decltype(member)::member_type>();
      if (field_name == "id") {
        field_type_sql = "INTEGER PRIMARY KEY AUTOINCREMENT";
      }
      field_definitions.push_back(
          std::format("    {} {}", field_name, field_type_sql));
    });

    if (field_definitions.empty()) {
      throw std::runtime_error(
          std::format("No fields found for type {}", table_name_str));
    }
...
public:
  ...
  template <typename T> void register_model() {
    std::string model_name = get_table_name<T>();

    if (std::find(registered_models.begin(), registered_models.end(),
                  model_name) != registered_models.end()) {
      std::cout << "Model " << model_name << " already registered, skipping..."
                << std::endl;
      return;
    }
...
inline std::expected<bool, std::string> SchemaManager::create_all_tables() {
  ...
  std::cout << "Creating " << create_table_sqls.size()
            << " tables in single batch..." << std::endl;
  ...
  for (size_t i = 0; i < registered_models.size(); ++i) {
    ...
    if (created_tables.find(table_name) != created_tables.end()) {
      std::cout << "Table " << table_name << " already created, skipping..."
                << std::endl;
      continue;
    }
  ...
  //         std::cerr << "Error during batch table creation: " <<
  //         success.error() << std::endl; return
  //         std::unexpected(success.error());
  //     }
  //     if (success.value()) {
  ...
  //                   << " tables created successfully in single database hit!"
  //                   << std::endl;
  //     } else {
  ...
  //     std::cerr << "Error during batch table creation: " << e.what() <<
  //     std::endl; return std::unexpected(e.what());
  // }
}
