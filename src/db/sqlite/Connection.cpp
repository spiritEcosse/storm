...
Connection::Connection(const std::string &db_name)
    : db(nullptr), transaction_active(false) {
  ...
  if (db) {
    ...
    if (transaction_active) {
      try {
        ...
        std::cerr << "Error rolling back transaction during connection close: "
                  << e.what() << std::endl;
      }
    }
...
Connection::Connection(Connection &&other) noexcept
    : db(other.db), transaction_active(other.transaction_active) {
  other.db = nullptr;
  ...
  if (this != &other) {
    ...
    if (db && transaction_active) {
      try {
        ...
        std::cerr << "Error rolling back transaction during move assignment: "
                  << e.what() << std::endl;
      }
    }
...
sqlite3 *Connection::get() const { return db; }

bool Connection::is_open() const { return db != nullptr; }
...
bool Connection::in_transaction() const { return transaction_active; }
