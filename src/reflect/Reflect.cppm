...
auto bind_variant(Statement& stmt, int param_index, const storm::SqlValue& value) 
    -> std::expected<void, std::string> {
    
    return std::visit([&](const auto& v) -> std::expected<void, std::string> {
...
struct type_descriptor {
    ...
    template<typename F>
    static consteval void for_each_member(F&& func) {
    ...
    template<typename F>
    static consteval void visit_members(T& obj, F&& func) {
    ...
    // New method that passes member descriptor directly to the callback (mutable version)
    template<typename F>
...
struct reflect {
    ...
    static consteval auto get_member_names_if(Predicate&& predicate) -> std::expected<std::vector<std::string>, std::string> {
    ...
    static consteval bool is_primary_key() {
    ...
    template<size_t I = 0>
    static consteval auto get_primary_key_member() {
