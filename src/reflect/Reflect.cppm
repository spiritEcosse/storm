...
export namespace refl {
    ...
    template <reflectable T> struct reflect {
        ...
        static auto get_member_value(const T& obj, std::string_view name) -> std::expected<std::any, std::string> {
            ...
            return result ? std::expected<std::any, std::string>{*result}
        ...
        static auto validate_field_names(std::span<const std::string> field_names) -> std::expected<void, std::string> {
            ...
            for (const auto& field : field_names) {
                if (std::ranges::find(valid_names, field) == valid_names.end()) {
        ...
        static auto to_map(const T& obj) -> std::expected<std::unordered_map<std::string, std::any>, std::string> {
            ...
            descriptor::visit_members(obj, [&](std::string_view name, const auto& value) {
                result[std::string{name}] = std::make_any<std::decay_t<decltype(value)>>(value);
            });
            return result;
        }
        ...
        static auto from_map(const std::unordered_map<std::string, std::any>& value_map)
            ...
            descriptor::for_each_member([&](auto member) {
                ...
                if (auto it = value_map.find(member_name); it != value_map.end()) {
                    ...
                    if (auto ptr = std::any_cast<MemberType>(&it->second)) {
                        if (auto result = member.set(obj, *ptr); !result) {
                            errors.push_back(std::format("Failed to set '{}': {}", member_name, result.error()));
                        }
                    } else {
                        errors.push_back(std::format("Type mismatch for '{}'", member_name));
                    }
                }
            });

            return errors.empty() ? std::expected<T, std::string>{obj}
                                  : std::unexpected{storm::utils::join(errors, "; ")};
        }
        ...
        static bool has_member(std::string_view name) {
            auto names = descriptor::get_member_names();
        ...
        template <size_t I> static auto get_member_type_info() -> std::string {
            static_assert(I < member_count(), "Member index out of bounds");
            return std::string{descriptor::template get_member<I>().type_name()};
        }
    };
