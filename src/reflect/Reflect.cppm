...
auto bind_variant(Statement &stmt, int param_index,
                  const storm::SqlValue &value)
    -> std::expected<void, std::string> {

  return std::visit(
      [&](const auto &v) -> std::expected<void, std::string> {
        return stmt.bind(param_index, v);
      },
      value);
}
...
template <size_t N> struct fixed_string {
  ...
  consteval const char *c_str() const { return value; }
...
struct member_descriptor {
  ...
  template <typename U>
  static consteval auto set(class_type &obj, U &&value)
      -> std::expected<void, std::string> {
    if constexpr (std::is_assignable_v<member_type &, U>) {
      ...
      return std::unexpected{
          std::format("Cannot assign value to member '{}'", get_name())};
    }
  }
...
struct type_descriptor {
  ...
  static consteval size_t get_member_count() { return member_count; }
  ...
  template <typename F>
  static consteval void for_each_member_with_name(F &&func) {
    []<size_t... Is>(std::index_sequence<Is...>, F &&f) {
      (f(std::get<Is>(members_tuple{}),
         std::get<Is>(members_tuple{}).get_name()),
       ...);
  ...
  template <typename F>
  static consteval void visit_members(const T &obj, F &&func) {
  ...
  template <typename F>
  static consteval void visit_members_with_descriptor(const T &obj, F &&func) {
    []<size_t... Is>(std::index_sequence<Is...>, const T &o, F &&f) {
      auto members = members_tuple{};
  ...
  // version)
  template <typename F>
  static consteval void visit_members_with_descriptor(T &obj, F &&func) {
    []<size_t... Is>(std::index_sequence<Is...>, T &o, F &&f) {
      auto members = members_tuple{};
      (f(std::get<Is>(members), std::get<Is>(members).get(o)), ...);
    }(std::make_index_sequence<member_count>{}, obj, std::forward<F>(func));
  ...
  static consteval auto find_member_index(std::string_view name)
      -> std::optional<size_t> {
...
template <reflectable T> struct reflect {
  ...
  static consteval std::string_view name() { return descriptor::get_name(); }
  ...
  static auto get_member_value(const T &obj, std::string_view name)
    ...
    return result.has_value()
               ? std::expected<std::any, std::string>{*result}
  ...
  static auto validate_field_names(std::span<const std::string> field_names)
    ...
    for (const auto &field_name : field_names) {
      bool found =
          std::ranges::any_of(valid_names, [&](std::string_view valid_name) {
    ...
    if (!invalid_fields.empty()) {
      return std::unexpected{
          std::format("Invalid field names: {}. Valid fields: {}",
  ...
  static auto to_map(const T &obj)
      -> std::expected<std::unordered_map<std::string, std::any>, std::string> {
    std::unordered_map<std::string, std::any> result;

    descriptor::visit_members(
        obj, [&](std::string_view member_name, const auto &value) {
          result[std::string{member_name}] =
              std::make_any<std::decay_t<decltype(value)>>(value);
        });
  ...
  static auto
  from_map(const std::unordered_map<std::string, std::any> &value_map)
    ...
    descriptor::for_each_member([&]<size_t I>(auto member) {
      ...
      if (auto it = value_map.find(member_name); it != value_map.end()) {
        ...
        if (auto ptr = std::any_cast<MemberType>(&it->second); ptr) {
          ...
          errors.push_back(
              std::format("Type mismatch for member '{}': cannot cast from any",
                          member_name));
        }
      }
  ...
  static auto to_json_string(const T &obj)
  ...
  static consteval std::string_view get_member_name(const MemberDesc &member) {
...
#define REFLECT_CLASS(ClassName, ...)                                          \
  template <> struct refl::type_info<ClassName> {                              \
    ...
    using descriptor =                                                         \
        refl::meta::type_descriptor<ClassName, #ClassName, __VA_ARGS__>;       \
  };
