...
QuerySet<Person> qs;
auto result = qs.where(fields::Person.age > 30).select().execute();
```
...
auto qs1 = qs.where(fields::Person.age > 30);
// qs1 type: QuerySet<Person, Connection, ComparisonExpr<Person::age, Greater, int>>

auto qs2 = qs1.where(fields::Person.name == "Alice");
...
queryset.where(fields::Person.age > 30).select();
