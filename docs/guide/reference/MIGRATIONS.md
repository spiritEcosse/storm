...
Storm integrates [Atlas](https://atlasgo.io) for database schema management. Models are auto-discovered via C++26 namespace reflection — any struct with `[[= storm::primary]]` in the target namespace is picked up automatically.
...
3. Checks each class for a `[[= storm::primary]]` annotation
...
|-------------------------------------------|-----------------------------------------|
| `[[= storm::primary]] int id;`        | `id INTEGER PRIMARY KEY`                 |
