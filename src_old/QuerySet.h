...
namespace orm {
    ...
    namespace detail {
        template <typename T> std::string getFullFieldName(const T& obj) {
            return obj.getFullFieldName();
        }

        template <> inline std::string getFullFieldName(const Function& obj) {
            return obj.getFullFieldName();
        }

        template <> inline std::string getFullFieldName(const BaseField* const& obj) {
            return obj->getFullFieldName();
        }

        template <> inline std::string getFullFieldName(const std::variant<const BaseField*, Function>& obj) {
            return std::visit([](const auto& value) { return getFullFieldName(value); }, obj);
        }
    } // namespace detail
    ...
    template <class T> class BaseQuerySet : public BaseQuerySetVirtual {
      public:
        ...
        template <typename U> void group_by_impl(U&& u) {
            groupByFields.emplace_back(std::forward<U>(u));
        }

        template <typename U, typename... Args> void group_by_impl(U&& u, Args&&... args) {
            groupByFields.emplace_back(std::forward<U>(u));
        ...
        [[nodiscard]] std::string aliasQueryMain() const {
            ...
            if (_one) {
                ...
                query += fmt::format(
                        "'{0}', COALESCE((SELECT json_agg({0}.*) FROM ( SELECT * FROM {0} "
                        ") as {0}), '[]'),",
                        _alias
                );
            }
        ...
        [[nodiscard]] std::string buildOrderFields() const {
            ...
            return format(
                    " ORDER BY {}",
                    fmt::join(
                            orderFields | std::views::transform([](const auto& pair) {
                                return fmt::format(
                                        "{} {}", detail::getFullFieldName(pair.first), pair.second ? "ASC" : "DESC"
                                );
                            }),
                            ", "
                    )
        ...
        [[nodiscard]] std::string createDistinctClause() const {
            ...
            return format(
                    "DISTINCT ON ({}) ",
                    fmt::join(
                            distinctInfo.distinctFields |
                                    std::views::transform([](const auto& field) { return field->getFullFieldName(); }),
                            ", "
                    )
            );
        }
        ...
        [[nodiscard]] std::string buildOnlyFields() const {
            return format(
                    "{}",
                    fmt::join(
                            onlyFields | std::views::transform([](const auto& fieldAlias) {
        ...
        template <typename U> void only_impl(const U& u) {
            onlyFields.emplace_back(u);
        }

        template <typename U, typename... Args> void only_impl(const U& u, Args... args) {
            onlyFields.emplace_back(u);
            only_impl(args...);
        }

        template <typename U> void distinct_impl(const U& u) {
            distinctInfo.distinctFields.emplace_back(u);
        }

        template <typename U, typename... Args> void distinct_impl(const U& u, const Args&... args) {
            distinctInfo.distinctFields.emplace_back(u);
            distinct_impl(args...);
        }

        template <typename U> void functions_impl(U&& u) {
            functionsSet.emplace_back(std::forward<U>(u));
        }

        template <typename U, typename... Args> void functions_impl(U&& u, Args&&... args) {
            functionsSet.emplace_back(std::forward<U>(u));
        ...
        void join_impl_core(
            ...
            if (it != mapFields.end()) {
                ...
                joinInfo.joins
                        .emplace_back(std::string(tableReference), std::move(alias), std::move(condition), joinType);
            }
        }
        ...
        template <class U>
        void join_impl(
                std::string&& cte, std::string&& alias, std::string&& addConditions, const JoinInfo::JoinType joinType
        ) {
        ...
        template <class U> void addCTE(BaseQuerySet<U>&& cte) {
        ...
        [[nodiscard]] std::string buildSelectOne() const {
            return fmt::format(
                    ...
                    _jsonFields.empty() ? fmt::format("{}{}", buildOnlyFields(), buildFunctions())
        ...
        [[nodiscard]] std::string buildOtherQueries() const {
            ...
            return format(
                    "WITH {}",
                    fmt::join(
                            otherQueries | std::views::transform([](const auto& query) {
        ...
        [[nodiscard]] std::string buildFunctions() const {
            ...
            return format(
                    ...
                    fmt::join(
                            functionsSet | std::views::transform([](const auto& function) { return function.toStr(); }),
                            ", "
                    )
            );
        }
        ...
        [[nodiscard]] std::string filter_impl() const {
            ...
            auto transformed =
                    filters | std::views::transform([](const WhereClause& filter) { return filter.serialize(); });
    ...
    template <class T> class QuerySet final : public BaseQuerySet<T> {
      ...
      public:
        ...
        explicit QuerySet(const std::string_view& alias, const bool doAndCheck = false, const bool returnInMain = true)
            : BaseQuerySet<T>(alias, doAndCheck, returnInMain) {}

        [[nodiscard]] std::string alias() const {
            return this->_alias;
        }

        template <typename... Args> QuerySet& group_by(Args&&... args) {
        ...
        QuerySet& join(QuerySet<U>&& other, std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(
                    std::move(other), std::move(alias), std::move(addConditions), JoinInfo::JoinType::INNER
            );
            return *this;
        }
        ...
        QuerySet& join(std::string&& cte, std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(
                    std::move(cte), std::move(alias), std::move(addConditions), JoinInfo::JoinType::INNER
            );
            return *this;
        }
        ...
        QuerySet& left_join(QuerySet<U>&& other, std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(
                    std::move(other), std::move(alias), std::move(addConditions), JoinInfo::JoinType::LEFT
            );
            return *this;
        }
        ...
        template <typename... Args> QuerySet& distinct(const Args&... args) {
        ...
        template <typename... Args> QuerySet& only(const Args&... args) {
            this->only_impl(args...);
            return *this;
        }
    ...
    class BuildComplexQueries final : public BaseClass {
      public:
        ...
        template <class U, typename... Args> static std::string addQuery_impl(U&& u, Args&&... args) {
        ...
        template <class U, typename... Args> static std::string addQueryMain_impl(U&& u, Args&&... args) {
            return fmt::format(
                    "{} {}", std::forward<U>(u).aliasQueryMain(), addQueryMain_impl(std::forward<Args>(args)...)
            );
        }

        template <typename... Args> static std::string buildQuery(Args&&... args) {
            std::string query = removeLastComma(fmt::format("WITH {}", addQuery_impl(std::forward<Args>(args)...)));
            return fmt::format(
                    " {} SELECT json_build_object({}) as result",
                    query,
