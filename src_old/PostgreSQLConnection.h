...
public:
     ...
     */
    explicit PostgreSQLConnection(const DBConfig& dbConfig) 
        : config(dbConfig), logger(Logger::getInstance()) {}
...
inline std::shared_ptr<PreparedStatement> PostgreSQLConnection::prepareStatement(const std::string& query) {
    ...
    return std::make_shared<PostgreSQLPreparedStatement>(
        std::dynamic_pointer_cast<PostgreSQLConnection>(shared_from_this()), 
        query
    );
}
