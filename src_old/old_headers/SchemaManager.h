...
// Legacy header placeholder; use C++23 modules (import storm.schema_manager).
import <string>;
import <vector>;
import <typeinfo>;
import <functional>;
...
namespace storm {
    class SchemaManager {
      ...
      public:
        explicit SchemaManager(std::shared_ptr<Connection> connection) : conn(std::move(connection)) {}
      ...
      private:
        ...
        template <typename T> std::string generate_create_table_sql() {
            ...
            if (field_definitions.empty()) {
                throw std::runtime_error(std::format("No fields found for type {}", get_table_name<T>()));
            }

            // Join field definitions with ",\n"
            std::string fields_joined;
            for (size_t i = 0; i < field_definitions.size(); ++i) {
                if (i)
                    fields_joined += ",\n";
                fields_joined += field_definitions[i];
            }
            return std::format("CREATE TABLE IF NOT EXISTS {} (\n{}\n);", table_name_str, fields_joined);
        }

        template <typename T> std::string get_sql_type() {
            using BaseType = std::decay_t<T>;

            if constexpr (std::is_same_v<BaseType, int> || std::is_same_v<BaseType, int32_t> ||
                          std::is_same_v<BaseType, int64_t> || std::is_same_v<BaseType, long> ||
                          std::is_same_v<BaseType, long long> || std::is_same_v<BaseType, size_t>) {
                return "INTEGER";
            } else if constexpr (std::is_same_v<BaseType, double> || std::is_same_v<BaseType, float> ||
                ...
                return "TEXT";
            } else if constexpr (std::is_same_v<BaseType, const char*> || std::is_same_v<BaseType, char*>) {
                return "TEXT";
            } else {
