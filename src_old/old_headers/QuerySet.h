...
// This legacy header is removed in favor of C++23 modules
// Legacy header placeholder; use C++23 modules (import storm.query_set).
// DEPRECATED: This header is being replaced by the C++23 module system.
// Please use 'import storm.query_set;' instead of including this header.
// Legacy support will be removed in a future version.

// Forward declarations and minimal compatibility layer
import <string>;
import <utility>;
import <vector>;
import <variant>;
...
// C++23-only: import the module unconditionally
import storm.query_set;

namespace storm {
    // Forward declarations of key types
    using ValueVariant           = std::variant<std::monostate, int, double, bool, std::string>;
    ...
    struct FieldAliasBase {
        ...
        explicit FieldAliasBase(std::string alias = "") : alias(std::move(alias)) {}
    ...
    template <auto MemberPtr> struct FieldAlias : public FieldAliasBase {
        // Constructor for string literals and std::string
        explicit FieldAlias(std::string alias = "") : FieldAliasBase(std::move(alias)) {}
    ...
    template <typename T> std::unique_ptr<FieldAliasBase> make_field_alias_unique(const T& field) {
    ...
    template <class T> class QuerySet {
      private:
        ...
        struct OrderFieldInfo {
            ...
            OrderFieldInfo(const OrderFieldInfo& other) : ascending(other.ascending), collation(other.collation) {
        ...
        [[nodiscard]] std::string aliasQueryMain() const {
            ...
            if (_one) {
                query += std::format("'{}', (SELECT * FROM {} ),", _alias);
            } else {
                query += std::format(
                        "'{}', COALESCE((SELECT json_agg({}.*) FROM ( SELECT * FROM {} ) as {}), '[]'),",
                        _alias,
                        _alias,
                        _alias,
                        _alias
        ...
        static void bind_sql_value(Statement& stmt, int param_index, const ValueType& value) {
            if constexpr (std::is_same_v<ValueType, std::string>) {
                ...
                std::visit([&stmt, param_index](const auto& v) { bind_sql_value(stmt, param_index, v); }, value);
            } else {
        ...
        template <auto Field> auto createFieldNameGenerator() const {
            return [this]() {
      ...
      public:
        ...
        QuerySet(const QuerySet& other)
            ...
            for (const auto& join : other.joinInfo.joins) {
                joinInfo.joins.emplace_back(join.tableName, join.alias, join.condition, join.type);
            }
        ...
        QuerySet& operator=(const QuerySet& other) {
            if (this != &other) {
                ...
                if (other._whereExpression) {
                    _whereExpression =
                            *other._whereExpression; // This will use Where's copy constructor for deep copying
                } else {
      ...
      private:
        std::vector<int>
        execute_insert(const std::vector<const T*>& obj_ptrs, const std::vector<std::string>& field_names) {
            if (obj_ptrs.empty())
                return {};
        ...
        std::string build_insert_sql(
                const std::vector<std::string>& field_names, size_t num_objects = 1, bool returning_id = false
        ) {
            if (field_names.empty())
                return "";

            std::vector<std::string> placeholders(field_names.size(), "?");
            std::string              single_group = std::format("({})", storm::utils::join(placeholders, ", "));
            ...
            return std::format(
                    ...
                    storm::utils::join(field_names, ", "),
                    storm::utils::join(value_groups, ", "),
                    returning_clause
            );
        }
      ...
      private:
        ...
        build_update_sql(const std::vector<std::string>& field_names, size_t num_objects) {
            if (field_names.empty() || num_objects == 0)
                return "";
            ...
            for (const auto& field : field_names) {
                std::string case_expr = std::format("{} = CASE id\n", field);
            ...
            return std::format(
                    "UPDATE {}\nSET\n    {}\nWHERE id IN ({});",
                    this->template get_table_name<T>(),
                    storm::utils::join(set_clauses, ",\n    "),
                    storm::utils::join(id_placeholders, ", ")
            );
        }

        std::expected<bool, std::string>
        execute_update(const std::vector<const T*>& obj_ptrs, const std::vector<std::string>& field_names) {
            if (obj_ptrs.empty())
                return std::unexpected("Empty object pointers vector");
      ...
      public:
        ...
        std::expected<bool, std::string> remove(const std::vector<T>& objs) {
            if (objs.empty())
                return std::unexpected("Empty objects vector");
      ...
      private:
        std::expected<std::string, std::string> build_delete_sql() {
            return std::format("DELETE FROM {} WHERE id = ?;", this->template get_table_name<T>());
        }

        std::expected<std::string, std::string> build_delete_by_condition_sql() {
            std::string sql = std::format("DELETE FROM {}", this->template get_table_name<T>());
      ...
      public:
        ...
        template <auto... Fields> QuerySet& group_by() {
      ...
      private:
        [[nodiscard]] std::string generateGroupBySQL() const {
            ...
            return std::format("GROUP BY {}", storm::utils::join(fieldStrings, ", "));
        }
      ...
      public:
        ...
        template <auto Field> QuerySet& order_by(Collation collation = Collation::NONE) {
            return add_order_field<Field>(true, collation);
        }

        // Modern compile-time version for multiple field-direction pairs
        template <auto Field, auto Direction, auto... Rest> QuerySet& order_by() {
            ...
            static_assert(
                    sizeof...(Rest) % 2 == 0, "Must provide field-direction pairs (field, bool, field, bool, ...)"
            );
        ...
        template <auto Field, auto Direction, auto Coll, auto... Rest> QuerySet& order_by_collate() {
            ...
            this->orderFields.emplace_back(std::make_unique<FieldAlias<Field>>(), Direction, Coll);
      ...
      private:
        ...
        template <auto Field> QuerySet& add_order_field(bool ascending, Collation collation) {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");

            // Add field to order criteria with specified direction and collation
            this->orderFields.emplace_back(std::make_unique<FieldAlias<Field>>(), ascending, collation);
        ...
        [[nodiscard]] std::string buildOrderFields() const {
            ...
            auto formattedFields = orderFields | std::ranges::views::transform([](const auto& orderInfo) {
                                       ...
                                       if (orderInfo.collation != Collation::NONE) {
                                           collation =
                                                   std::format(" COLLATE {}", collation_to_sql(orderInfo.collation));
                                       }

                                       return std::format("{}{} {}", fieldName, collation, direction);
                                   });
            fieldStrings.assign(formattedFields.begin(), formattedFields.end());

            return std::format(" ORDER BY {}", storm::utils::join(fieldStrings, ", "));
        }
      ...
      public:
        ...
        template <auto MemberPtr, typename Value> QuerySet& where(Value&& value, storm::Op op = storm::Op::EQ);

        // Special methods for complex operators
        // Compile-time member pointer version (C++23 NTTP)
        template <auto MemberPtr, typename Container> QuerySet& where_in(const Container& values);

        // Where between (compile-time member pointer version - C++23 NTTP)
        template <auto MemberPtr, typename T1, typename T2> QuerySet& where_between(T1&& value1, T2&& value2);

        // Where null (compile-time member pointer version - C++23 NTTP)
        template <auto MemberPtr> QuerySet& where_null();

        // Where not null (compile-time member pointer version - C++23 NTTP)
        template <auto MemberPtr> QuerySet& where_not_null();

        // Where like (compile-time member pointer version - C++23 NTTP)
        template <auto MemberPtr, typename Value> QuerySet& where_like(Value&& pattern);
        ...
        template <auto MemberPtr>
        QuerySet& where_date_range(const std::string& start_date, const std::string& end_date);
      ...
      public:
        ...
        // JOIN
        // =============================
        template <class U> QuerySet& join(std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(std::move(alias), std::move(addConditions), JoinInfo::JoinType::INNER);
            return *this;
        }

        template <class U> QuerySet& left_join(std::string&& alias = "", std::string&& addConditions = "") {
        ...
        QuerySet& left_join(std::string&& cte, std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(
                    std::move(cte), std::move(alias), std::move(addConditions), JoinInfo::JoinType::LEFT
            );
            return *this;
        }

        template <class U> QuerySet& right_join(std::string&& alias = "", std::string&& addConditions = "") {
        ...
        QuerySet& right_join(std::string&& cte, std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(
                    std::move(cte), std::move(alias), std::move(addConditions), JoinInfo::JoinType::RIGHT
            );
            return *this;
        }
        ...
        QuerySet& right_join(QuerySet<U>&& other, std::string&& alias = "", std::string&& addConditions = "") {
            this->template join_impl<U>(
                    std::move(other), std::move(alias), std::move(addConditions), JoinInfo::JoinType::RIGHT
            );
            return *this;
        }
      ...
      private:
        template <class U>
        void join_impl_core(std::string&& alias, std::string&& addConditions, JoinInfo::JoinType joinType) {
            std::string condition;
            ...
            if (condition.empty() && addConditions.empty()) {
                throw std::runtime_error(
                        "Could not determine join condition automatically. Please provide explicit join conditions."
                );
            }
            ...
            for (const auto& [tableName, alias, condition, type] : joinInfo.joins) {
                if (!tableName.empty()) {
                    sql += std::format(
                            ...
                            tableName,
                            alias.empty() ? "" : std::format(R"( AS "{}")", alias),
                            condition
                    );
                }
            }
        ...
        template <typename SourceType, typename TargetType> std::string determine_join_condition() {
            ...
            if (it != field_names.end()) {
                return std::format(
                        R"("{}"."id" = "{}"."{}")", get_table_name<TargetType>(), get_table_name<SourceType>(), *it
                );
            }
      ...
      public:
        ...
        template <auto... Fields> QuerySet& only() {
            static_assert((std::is_member_pointer_v<decltype(Fields)> && ...), "All fields must be member pointers");
        ...
        template <auto Field> QuerySet& only(std::string_view alias) {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");
      ...
      public:
        ...
        bool should_use_bulk_population() const {
            return this->onlyFields.empty() && this->distinctFields.empty() && this->functionsSet.empty();
        }
      ...
      private:
        [[nodiscard]] std::string limit_impl() const {
            ...
            if (_limit && _offset) {
                return std::format(" LIMIT {} OFFSET {}", _limit, _offset);
            } else if (_limit) {
                return std::format(" LIMIT {}", _limit);
            } else {
                // SQLite requires LIMIT when using OFFSET
                // Use LIMIT -1 to get all rows with an offset
                return std::format(" LIMIT -1 OFFSET {}", _offset);
            }
        }
      ...
      public:
        ...
        // DISTINCT
        // =============================
        template <auto... Fields> QuerySet& distinct() {
            static_assert((std::is_member_pointer_v<decltype(Fields)> && ...), "All fields must be member pointers");
      ...
      private:
        ...
        [[nodiscard]] std::string buildFieldsClause() const {
            ...
            if (!distinctFields.empty() && onlyFields.empty()) {
                ...
                auto field_strings =
                        distinctFields |
                        std::views::transform([](const auto& field_alias) { return field_alias->getFullFieldName(); }) |
                        std::ranges::to<std::vector<std::string>>();
                ...
                auto field_strings =
                        onlyFields | std::views::transform([](const auto& field_alias) {
                            return field_alias->alias.empty()
                                           ? field_alias->getFullFieldName()
                                           : std::format(
                                                     "{} AS {}", field_alias->getFullFieldName(), field_alias->alias
                                             );
                        }) |
                        std::ranges::to<std::vector<std::string>>();
        ...
        template <refl::reflectable T> [[nodiscard]] std::string build_default_fields_clause() const {
            std::vector<std::string> field_strings;
            field_strings.reserve(refl::reflect<T>::member_count());

            const std::string table_name = get_table_name<T>();

            refl::reflect<T>::for_each_member([&]<size_t I>(auto member) {
                const std::string field_name{member.get_name()};
                const std::string formatted_field = storm::utils::formatFieldName(table_name, field_name);
                field_strings.emplace_back(std::format(R"({} AS "{}")", formatted_field, field_name));
            });
      ...
      public:
        ...
        template <auto Field> QuerySet& max(std::string_view alias = "") {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");
            ...
            functions(Function(std::format("MAX({}) AS {}", field->getFullFieldName(), actual_alias)));
            return *this;
        }

        // MIN aggregate function
        template <auto Field> QuerySet& min(std::string_view alias = "") {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");
            auto        field = std::make_unique<FieldAlias<Field>>();
            std::string actual_alias(alias);
            if (actual_alias.empty()) {
            ...
            // We want to keep any existing onlyFields to allow selecting both fields and aggregate functions
            functions(Function(std::format("MIN({}) AS {}", field->getFullFieldName(), actual_alias)));
            return *this;
        }

        // AVG aggregate function
        template <auto Field> QuerySet& avg(std::string_view alias = "") {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");
            ...
            // We want to keep any existing onlyFields to allow selecting both fields and aggregate functions
            functions(Function(std::format("AVG({}) AS {}", field->getFullFieldName(), actual_alias)));
            return *this;
        }

        // COUNT aggregate function
        template <auto Field> QuerySet& count(std::string_view alias = "") {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");
            auto        field = std::make_unique<FieldAlias<Field>>();
            std::string actual_alias(alias);
            if (actual_alias.empty()) {
            ...
            // We want to keep any existing onlyFields to allow selecting both fields and aggregate functions
            functions(Function(std::format("COUNT({}) AS {}", field->getFullFieldName(), actual_alias)));
            return *this;
        }

        // SUM aggregate function
        template <auto Field> QuerySet& sum(std::string_view alias = "") {
            static_assert(std::is_member_pointer_v<decltype(Field)>, "Field must be a member pointer");
            ...
            functions(Function(std::format("SUM({}) AS {}", field->getFullFieldName(), actual_alias)));
            return *this;
        }

        // Helper function to check member pointer at compile time
        template <auto Field> static constexpr bool check_member_pointer() {
        ...
        template <auto Field> auto avg_value() -> std::expected<double, std::string> {
        ...
        template <auto Field> auto count_value() -> std::expected<int, std::string> {
        ...
        template <auto Field> auto sum_value() -> std::expected<double, std::string> {
            using FieldType = typename member_pointer_traits<decltype(Field)>::type;
        ...
        template <auto FirstField, auto... RestFields>
        auto group_concat_value(
                std::string_view separator = ",", std::string_view fieldSeparator = " ", bool distinct = false
        ) -> std::expected<std::string, std::string> {
            return execute_aggregate_query<std::string>(
                    [separator, fieldSeparator, distinct](auto& qs) {
                        qs.template group_concat<FirstField, RestFields...>("", separator, fieldSeparator, distinct);
                    },
                    "group_concat"
            );
        }
        ...
        auto group_concat_value(
            ...
            return execute_aggregate_query<std::string>(
                    [separator, fieldSeparator, distinct](auto& qs) {
                        qs.template group_concat<FirstField, RestFields..., OrderField>(
                                "", separator, fieldSeparator, distinct
                        );
                    },
                    "group_concat"
            );
      ...
      private:
        ...
        prepare_group_concat(std::string_view alias, std::string_view fieldSeparator) {
            ...
            std::string actual_alias =
                    alias.empty() ? std::format("group_concat_{}", firstField->getFieldName()) : std::string(alias);
        ...
        std::string build_field_expression(std::string_view fieldSeparator) const {
            ...
            (
                    [&field_expr, &fieldSeparator]<auto Field>() {
                        auto field = std::make_unique<FieldAlias<Field>>();
                        field_expr = std::format("{}||'{}'||{}", field_expr, fieldSeparator, field->getFullFieldName());
        ...
        QuerySet& group_concat_impl(
            ...
            if (!distinct) {
                function_str += std::format(", '{}'", separator);
            }

            function_str += std::format(") AS {}", actual_alias);
        ...
        template <auto OrderField, auto FirstField, auto... RestFields>
        QuerySet& group_concat_with_order_impl(
                std::string_view alias, std::string_view separator, std::string_view fieldSeparator, bool distinct
        ) {
        ...
        std::expected<ReturnType, std::string> execute_aggregate_query(
            ...
            if (row.get_column_count() == 0) [[unlikely]] {
                return std::unexpected(std::format("{} column not found", error_prefix));
            }
            ...
            if (row.get_column_type(0) == SQLITE_NULL) [[unlikely]] {
                if constexpr (std::is_same_v<ReturnType, std::string>) {
                    return std::string{};
                } else if constexpr (std::is_same_v<ReturnType, int> || std::is_same_v<ReturnType, int64_t> ||
                                     std::is_integral_v<ReturnType>) {
                    return 0;
                } else if constexpr (std::is_same_v<ReturnType, double> || std::is_floating_point_v<ReturnType>) {
                    return 0.0;
                } else {
            ...
            if constexpr (!std::is_same_v<decltype(value_extractor), std::nullptr_t>) {
                ...
                try {
                    ...
                    return std::unexpected<std::string>(
                            std::format("Unexpected column type for {} result", error_prefix)
                    );
                }
            }
        }

        template <typename... Args> QuerySet& functions(Args&&... args) {
            // Reserve capacity
