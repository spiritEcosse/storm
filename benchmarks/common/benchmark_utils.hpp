...
namespace storm::benchmark {
    ...
    namespace db_utils {
        ...
        constexpr const char* PERSON_TABLE_SQL = "CREATE TABLE Person ("
    ...
    namespace data_utils {
        ...
        inline std::vector<PersonType> generate_simple_test_data(int count, int start_id = 1) {
            ...
            for (int i = 0; i < count; ++i) {
                persons.push_back({start_id + i, "Person" + std::to_string(start_id + i), 20 + ((start_id + i) % 50)});
            }
            return persons;
        }
        ...
        inline std::vector<PersonType> generate_test_data_range(size_t count, int start_id) {
            ...
            for (size_t i = 0; i < count; ++i) {
                persons.push_back({start_id + static_cast<int>(i), "Test" + std::to_string(i), 25});
            }
            return persons;
        }
    ...
    namespace format_utils {
        ...
        inline void print_benchmark_result(
            ...
            std::cout << "  Average per " << unit.substr(0, unit.find('s')) << ": " << std::fixed
                      << std::setprecision(4) << (total_time / successful_ops) << " ms" << std::endl;
