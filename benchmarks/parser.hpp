...
template<size_t N>
constexpr ConstexprString<N> parse_string(std::string_view json, size_t& pos) {
    skip_whitespace(json, pos);
...
template<size_t N>
constexpr std::array<BenchmarkTest, N> parse_tests(std::string_view json) {
