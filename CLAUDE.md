...
**Known gap (#572)**: plain `insert(uuid_model)` still defaults to `ReturnId::Yes` and emits a
`RETURNING <uuid_pk>` extracted as an integer — spell `insert<ReturnId::No>` on a UUID-PK model.
