...
**Known limitation (#586)**: every single-column INTEGER key is classified DB-generated, so a
**caller-assigned integer key cannot be expressed** — `insert()` drops the caller's value. The DDL
already permits one (PG `GENERATED BY DEFAULT`, not `ALWAYS`; SQLite rowid alias), and
`storm::primary` vs `primary_autoincrement` differ in one DDL keyword only — no statement-side code
reads `is_primary_autoincrement`. Needs a declaration-level opt-in: an integer `id == 0` cannot be
distinguished from "unset" the way an empty UUID can.
