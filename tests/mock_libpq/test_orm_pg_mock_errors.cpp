...
namespace {
    ...
    // ============================================================================
    // schema.cppm lines 58-73: detail::replace_all and detail::apply_pg_transforms
    // ============================================================================
    ...
    TEST_F(PgSchemaDetailTest, ApplyPgTransformsConvertsAllTypes) {
        ...
        std::string       pg_sql     = storm::orm::schema::detail::apply_pg_transforms(sqlite_sql);
        EXPECT_NE(pg_sql.find("GENERATED BY DEFAULT AS IDENTITY"), std::string::npos);
        EXPECT_NE(pg_sql.find("BYTEA"), std::string::npos);
        EXPECT_NE(pg_sql.find("DOUBLE PRECISION"), std::string::npos);
        EXPECT_NE(pg_sql.find("BIGINT"), std::string::npos);
        EXPECT_EQ(pg_sql.find("AUTOINCREMENT"), std::string::npos);
