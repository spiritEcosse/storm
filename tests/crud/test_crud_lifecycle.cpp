...
TYPED_TEST(QuerySetCrudLifecycleTest, FullLifecycle) {
    ...
    auto alice_result = qs.where(fields::Person.id == static_cast<int>(id_alice.value())).select().execute();
...
TYPED_TEST(QuerySetCrudLifecycleTest, AllFieldTypesLifecycle) {
    ...
    auto rows = qs.where(fields::Person.id == static_cast<int>(id.value())).select().execute();
    ...
    auto updated_rows = qs.where(fields::Person.id == static_cast<int>(id.value())).select().execute();
...
TYPED_TEST(QuerySetCrudLifecycleTest, FilteredLifecycle) {
    ...
    auto active = qs.where(fields::Person.is_active == true).select().execute();
    ASSERT_TRUE(active.has_value());
    EXPECT_EQ(static_cast<int>(active.value().size()), 2) << "Should have 2 active persons";

    // 3. Erase only persons aged < 21
    auto young = qs.where(fields::Person.age < 21).select().execute();
