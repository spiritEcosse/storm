...
namespace {
    ...
    TYPED_TEST_SUITE(FkColumnNameTest, DatabaseTypes);
    ...
    TYPED_TEST(FkColumnNameTest, GroupByPlainWithCountDistinctFkExecutes) {
        QuerySet<Message, TypeParam> qs;
        auto                         result = qs.template group_by<fields::Message.value>()
                              .template count_distinct<fields::Message.sender>()
                              .execute();
    ...
    static_assert(
            storm::meta::is_single_column_member(storm::meta::selector_info<fields::Message.sender>()),
    ...
    static_assert(
            storm::meta::is_single_column_member(storm::meta::selector_info<fields::Person.age>()),
            "an ordinary column is always single-column"
    );
    // Shipment::line → 2-part composite PK (order_id, product_id): rejected.
    static_assert(
            !storm::meta::is_single_column_member(storm::meta::selector_info<fields::Shipment.line>()),
            "an FK to a composite-PK target spans several columns and has no <member>_id"
    );
    static_assert(
            storm::meta::column_span_of(storm::meta::selector_info<fields::Shipment.line>()) == 2,
    ...
    static_assert(
            storm::meta::column_span_of(^^LedgerEntryRef::ledger) == 3,
    ...
    static_assert(
            storm::orm::statements::OrderBySelectorsAreSingleColumn<fields::Message.sender>,
            "ORDER BY <single-PK fk> stays available"
    );
    static_assert(
            !storm::orm::statements::OrderBySelectorsAreSingleColumn<fields::Shipment.line>,
    ...
    static_assert(
            storm::orm::statements::OrderBySelectorsAreSingleColumn<fields::Person.age, false>,
    ...
    static_assert(
            storm::orm::statements::SingleColumnSelector<fields::Message.sender>,
            "COUNT(DISTINCT <single-PK fk>) stays available"
    );
    static_assert(
            !storm::orm::statements::SingleColumnSelector<fields::Shipment.line>,
            "COUNT(DISTINCT <composite fk>) is refused at the call site"
    );
    static_assert(
            !storm::orm::statements::SingleColumnSelector<fields::OptionalShipment.line>,
    ...
    static_assert(
            !CanOrderBy<ShipmentQs, fields::Shipment.line>,
            "QuerySet::order_by must REJECT a composite-PK FK — fails if the requires-clause is dropped"
    );
    static_assert(
            CanCountDistinct<MessageQs, fields::Message.sender>, "COUNT(DISTINCT) on a single-PK FK must still compile"
    );
    static_assert(
            !CanCountDistinct<ShipmentQs, fields::Shipment.line>,
    ...
    TYPED_TEST_SUITE(NonFkSqlParityTest, DatabaseTypes);
    ...
    TYPED_TEST(NonFkSqlParityTest, OrderByPlainMemberSqlUnchanged) {
        ...
        EXPECT_EQ(
                qs.template order_by<fields::Person.name>().select().sql(),
                std::format(
                        "SELECT id, name, age, salary, is_active, years_experience, department, score, nickname,"
    ...
    TYPED_TEST(NonFkSqlParityTest, AggregateOnPlainMemberSqlUnchanged) {
        QuerySet<Person, TypeParam> qs;
        EXPECT_EQ(
                qs.template count_distinct<fields::Person.department>().sql(),
    ...
    TYPED_TEST(NonFkSqlParityTest, GroupByPlainMemberSqlUnchanged) {
        QuerySet<Person, TypeParam> qs;
        EXPECT_EQ(
                qs.template group_by<fields::Person.department>().count().sql(),
