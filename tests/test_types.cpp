  ...
  protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(conn.execute(
                "CREATE TABLE IntTypes (id INTEGER PRIMARY KEY AUTOINCREMENT, big_num INTEGER, small_num INTEGER)"
        ).has_value());
    }
    void TearDown() override { QuerySet<IntTypes>::clear_default_connection(); }
};
...
class FloatTypesInsertUpdateTest : public ::testing::Test {
  protected:
    ...
    void TearDown() override { QuerySet<FloatTypes>::clear_default_connection(); }
};
...
class MixedTypesInsertUpdateTest : public ::testing::Test {
  protected:
    ...
    void TearDown() override { QuerySet<MixedTypes>::clear_default_connection(); }
};
...
class OptTypesInsertUpdateTest : public ::testing::Test {
  protected:
    ...
    void TearDown() override { QuerySet<OptTypes>::clear_default_connection(); }
};
...
class DataTypesInsertUpdateTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(conn.execute(
                "CREATE TABLE DataTypes (id INTEGER PRIMARY KEY AUTOINCREMENT, binary BLOB, label TEXT)"
        ).has_value());
    }
    void TearDown() override { QuerySet<DataTypes>::clear_default_connection(); }
};
...
TEST_F(DataTypesInsertUpdateTest, InsertBatchBlobs) {
    QuerySet<DataTypes> qs;
    std::vector<DataTypes> batch = {
        {1, {0x01}, "blob1"},
        {2, {0x02, 0x03}, "blob2"},
        {3, {0x04, 0x05, 0x06}, "blob3"}
    };
