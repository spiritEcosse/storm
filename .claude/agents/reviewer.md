...
- For many-to-many (#203 model/schema; #391 execution): `many_to_many`/`many_to_many_through<T>` are `ManyToMany<>` class-template annotations, not FieldAttr values; m2m container members must stay filtered out of `all_members_`/`field_count_`. Eager load is a TWO-query predicate-pushdown (`execute_m2m_2query`): Q1 base entities + Q2 `(owner_pk, related.*) WHERE owner_id IN (<same base subquery>)`, stitched by a pk→entity hash map, both in one transaction. WHERE/ORDER/LIMIT live in Q1 AND inside Q2's IN-subquery so both select the same base set. INNER drops zero-relation entities post-stitch; LEFT keeps them. NO outer ORDER BY / pk-adjacency contract (deleted in #391 — the stitch is a hash map). `count()`/aggregates still use `get_complete_sql` (3-table join) to count (base,related) pairs
