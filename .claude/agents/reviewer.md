...
- For foreign keys (#431): the FK marker is the class-template annotation `fk<>` / `fk<RefAction::...>`, a class-template annotation (not a flag object) — every FK check must route through `meta::is_fk_field` (an `Fk<>` annotation-TYPE check in the `storm_orm_field_attr` leaf), never a raw `== fk<>` comparison. The `ON DELETE` clause is emitted only by `fk_on_delete_action_of` (Restrict → no clause, byte-identical pre-#431 DDL); `fk<RefAction::SetNull>` on a non-`std::optional` FK MUST stay a compile error via `ModelFkPoliciesValid<T>`; `ON UPDATE` is intentionally not emitted. Buffer estimators (`fk_references_len`/`max_fk_references_len`, `col_def_buffer`'s `+2` `\0`-guard) must fit the longest emitted `ON DELETE` clause.
- For many-to-many (#203 model/schema; #391 execution; #392 multi-relation): `many_to_many<>`/`many_to_many_through<T>` are `ManyToMany<Through, JunctionOnDelete>` class-template annotations, not flag objects; m2m container members must stay filtered out of `all_members_`/`field_count_`. The auto-junction `ON DELETE` defaults to CASCADE both sides, overridable via `many_to_many<RefAction::...>` (#431, `m2m_junction_on_delete_of` reads template arg [1]). Eager load is a TWO-query predicate-pushdown (`execute_m2m_2query`): Q1 base entities + one Q2 PER relation `(owner_pk, related.*) WHERE owner_id IN (<same base subquery>)`, stitched by one pk→entity hash map, all in one transaction (`JoinStatementWrapper::m2m_relations` vector of per-relation descriptors). WHERE/ORDER/LIMIT live in Q1 AND inside every Q2's IN-subquery so all select the same base set. INNER drops entities empty in ANY inner relation post-stitch; LEFT keeps them. Duplicate m2m fields / FK+m2m mixes in one `join<>()` call must stay compile-time rejected (`JoinableFields`); schema must create one junction table per auto-m2m field. NO outer ORDER BY / pk-adjacency contract (deleted in #391 — the stitch is a hash map). `count()`/aggregates still use `get_complete_sql` (chained junction join) — (base,related) pairs for one relation, cartesian tuples for several
