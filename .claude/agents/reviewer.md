...
- Ensure primary key fields are marked with `[[=storm::primary]]` (or `primary_autoincrement` for the SQLite never-reuse opt-in, #379); both are treated as a PK via `meta::is_primary_member`
...
- Upsert (#205/#458): `insert(p).on_conflict<Target...>().update<Cols...>(proto)`/`.nothing()`. Conflict target MUST be a single `storm::unique` field, matching `UniqueIndex<...>`, or the full PK column set — in declaration order — of a key INSERT actually EMITS (#503/#585: composite, or a single-column `storm::UUID`; a strict subset of a composite key is rejected at compile time, and a DB-generated single INTEGER key stays rejected because INSERT omits it so it can never collide). Verify return types: single-target `DO UPDATE` → `std::expected<int64_t, Error>`, `DO NOTHING` → `std::expected<std::optional<int64_t>, Error>` (nullopt when the row already existed); composite-PK **or UUID-PK** target → `std::expected<void, Error>` for BOTH (#502/#572 — neither key is DB-generated, so there is no RETURNING and DO NOTHING's skip signal is unavailable there; the gate is `BaseStatement::pk_is_db_generated_()`, and the upsert terminals read the MODEL SHAPE — `ReturnId` does not propagate through `on_conflict()`). Empty-WHERE refusal does not apply (single-row, INSERT-driven)
