...
Does a **fresh worktree** (no `build/` cache, no CPM download cache) still configure and build? New targets, new GLOB patterns, new generated files, and new experimental-feature UUIDs are the usual breakers (#521, #489). `commit.sh`'s self-heal (#489: configure + build `build/release` and `build/debug` before tidy/ctest; since #557 the release side builds only clang-tidy's prerequisites — storm BMIs, the synthesized-module BMIs, the two mock binaries — with a full-build fallback whenever it cannot prove that suffices) must still cover both the git-hook path (`.githooks/pre-commit` → `exec ./commit.sh`) and a manual `./commit.sh` invocation — check that this diff hasn't added a step that runs before the self-heal completes, or a new prerequisite the self-heal doesn't build.
